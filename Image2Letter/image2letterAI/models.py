...
class TypeRNet(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx) -> STEP_OUTPUT | None:
        ...
        if batch_idx % 6 == 0:      
            grid_in = torchvision.utils.make_grid(convert_rgb_tensor_for_plot(img_in[:4])).cpu().numpy()
            grid_out = torchvision.utils.make_grid(convert_gray_tensor_for_plot(out_img[:4])).cpu().numpy()
